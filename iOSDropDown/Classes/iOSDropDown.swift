...
//  Changes by Nayan Dave on 30/05/2022
...
open class DropDown : UITextField {
    ...
    @IBInspectable public var listHeight: CGFloat = UIScreen.main.bounds.height*0.5 {
        didSet {
    ...
    // Variables
    fileprivate var tableheightX: CGFloat = 250
...
//                    }
    ...
    public func showList() {
        ...
        if listHeight > rowHeight * CGFloat(dataArray.count) {
            self.tableheightX = (rowHeight * CGFloat(dataArray.count)) + 10
        } else {
...
//        let height = (self.parentController?.view.frame.height ?? 0) - (self.pointToParent.y + self.frame.height + 5)
        let y = self.pointToParent.y+self.frame.height+5
//        if height < (keyboardHeight+tableheightX){
//            y = self.pointToParent.y - tableheightX
//        }
        // Commented Above Code to avoid Upward direction which happens often when keyboard is open and tableheight is reaching the keyboard which causes loss of Visible rows & looks buggy
...
extension DropDown: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cell == nil {
            ...
            cell!.separatorInset = UIEdgeInsets(top: 2.5, left: 10, bottom: 2.5, right: 10)
        }
